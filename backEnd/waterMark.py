...
class Watermarker(object):
    ...
    def __init__(
            ...
            angle=30,
            color='#8B8B1B',
            font_file='/home/walkiiiy/SnapBird/backEnd/青鸟华光简琥珀.ttf',
        ...
        if self.django_support:
            ...
            self.font_file = os.path.join(
                settings.BASE_DIR, 'contrib', 'image', 'font', font_file)
        else:
            self.font_file = os.path.join(
                os.path.abspath('.'), 'font', font_file)
    ...
    def _add_mark_striped(self):
        ...
        # 计算斜边长度
        c = int(math.sqrt(
            origin_image.size[0] * origin_image.size[0] + origin_image.size[1] * origin_image.size[1]))
        ...
        origin_image.paste(
            watermark_mask,  # 大图
            (int((origin_image.size[0] - c) / 2),
             int((origin_image.size[1] - c) / 2)),  # 坐标
...
if __name__ == '__main__':
    imagePath = '/home/walkiiiy/tests/test.png'
    Watermarker(imagePath, 'mark text', WatermarkerStyles.CENTRAL).show()
    Watermarker(imagePath, 'mark text',
                WatermarkerStyles.CENTRAL).save('path_to_save')
    image = Watermarker(imagePath, 'mark text',
                        WatermarkerStyles.CENTRAL).image
